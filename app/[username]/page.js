...
async function getData(username) {
  ...
  let res = null;
  if (!token) {
    res = await fetch('https://socialmediajavaapi.onrender.com/api/v1/users/profile/'+username);
  }else{
    const decoded = jwtDecode(token);
    res = await fetch('https://socialmediajavaapi.onrender.com/api/v1/users/profile/'+username+"?userId="+decoded.id);
  }
  if (!res.ok) {
