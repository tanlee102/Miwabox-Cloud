...
async function getData(sortingIndex, pageNumber = 0, lastId = null, tagname = null, title = null) {
  ...
  if (tagname) {
    url = `http://8.219.96.109/api/v1/posts/topByTag?limit=${limit}&tagName=${tagname}`;
  } else if (title) {
    url = `http://8.219.96.109/api/v1/posts/topByTitle?limit=${limit}&keyword=${title}`;
  } else {
    switch (sortingIndex) {
      case 1: // Top Posts
        url = `http://8.219.96.109/api/v1/posts/orderByTotalLikes?pageNumber=${pageNumber}&pageSize=${pageSize}`;
        break;
      case 2: { // Chronology (was case 0)
        url = `http://8.219.96.109/api/v1/posts/orderById?limit=${limit}`;
      ...
      case 0: { // Followed (was case 2)
        ...
        if (token) {
          ...
          if (decoded?.id) {
            url = `http://8.219.96.109/api/v1/posts/followed?size=${limit}&page=${pageNumber}&userId=${decoded.id}`;
            break;
          }
        }else{
          setSortingIndex(2)
      ...
      default:
        url = `http://8.219.96.109/api/v1/posts`;
    }
