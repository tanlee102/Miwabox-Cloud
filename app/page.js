...
async function getData(sortingIndex, pageNumber = 0, lastId = null, tagname = null, title = null) {
  ...
  if (tagname) {
    url = `https://socialmediajavaapi.onrender.com/api/v1/posts/topByTag?limit=${limit}&tagName=${tagname}`;
  } else if (title) {
    url = `https://socialmediajavaapi.onrender.com/api/v1/posts/topByTitle?limit=${limit}&keyword=${title}`;
  } else {
    switch (sortingIndex) {
      case 1: // Top Posts
        url = `https://socialmediajavaapi.onrender.com/api/v1/posts/orderByTotalLikes?pageNumber=${pageNumber}&pageSize=${pageSize}`;
        break;
      case 2: { // Chronology (was case 0)
        url = `https://socialmediajavaapi.onrender.com/api/v1/posts/orderById?limit=${limit}`;
      ...
      case 0: { // Followed (was case 2)
        ...
        if (token) {
          ...
          if (decoded?.id) {
            url = `https://socialmediajavaapi.onrender.com/api/v1/posts/followed?size=${limit}&page=${pageNumber}&userId=${decoded.id}`;
            break;
          }
        }else{
          setSortingIndex(2)
      ...
      default:
        url = `https://socialmediajavaapi.onrender.com/api/v1/posts`;
    }
