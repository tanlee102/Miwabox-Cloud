...
const EditUserName = ({isDisplay,setIsDisplay, textIn, setTextIn}) => {
    ...
    const checkUserNameBtn = async () =>{
        if (textIn.trim().length >= 2) {
            try {
              const response = await axios.get(`https://socialmediajavaapi.onrender.com/api/v1/users/${encodeURIComponent(textIn)}/exists`);
              if (response.data) {
    ...
    const updateUserNameBtn = async () => {
        ...
        try {
          const response = await axios.post('https://socialmediajavaapi.onrender.com/api/v1/users/update/username', 
            { username: textIn }, 
            {
              headers: {
