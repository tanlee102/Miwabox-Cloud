      ...
      conFirmFun("Management", "Do you want to restrict this post?", async () => {
        ...
        setTimeout(async () => {
          try {
            ...
            const response = await axios.post(`https://socialmediajavaapi.onrender.com/api/v1/posts/toggle-restrict/${postId}`, {}, {
              headers: {
  ...
  const mutePost = () => {
      ...
      conFirmFun("Management", "Do you want to mute this post?", async () => {
        ...
        setTimeout(async () => {
          try {
            const token = Cookies.get('token');
            const response = await axios.post(`https://socialmediajavaapi.onrender.com/api/v1/posts/toggle-mute/${postId}`, {}, {
              headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
              }
  ...
  const deletePost = () => {
      ...
      conFirmFun("Management", "Do you want to delete this post?", async () => {
          ...
          setTimeout(async () => {
              try {
                  const token = Cookies.get('token');
                  const response = await axios.delete(`https://socialmediajavaapi.onrender.com/api/v1/posts/${postId}`, {
                      headers: {
                          'Authorization': `Bearer ${token}`,
                          'Content-Type': 'application/json'
                      }
