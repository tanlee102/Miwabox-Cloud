...
const loadData = async (userId, setMyData) => {
  ...
  try {
    const response = await axios.get(`https://socialmediajavaapi.onrender.com/api/v1/users/${userId}`);
...
const DetailAccount = () => {
  ...
  const updateAccount = async () => {
    ...
    try {
      const response = await axios.post('https://socialmediajavaapi.onrender.com/api/v1/users/profile/update', 
        { bio : bio }, 
        {
          headers: {
            'Authorization': `Bearer ${token}`,
