...
async function getNotification( userId) {
    const res = await fetch('https://socialmediajavaapi.onrender.com/api/v1/notification/users/'+userId+'/unread-count');
    if (!res.ok) {
...
async function getUnreadMessages(userId) {
    const res = await fetch('https://socialmediajavaapi.onrender.com/api/conversations/unread/count?userId='+userId);
    if (!res.ok) {
        console.log('Failed to fetch data');
        return null;
    }else{
