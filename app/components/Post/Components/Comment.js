...
const Comment = ({postId, setComments, comments, isMuted=false }) => {
    ...
    useEffect(() => {
      ...
      if(postId){
        axios.get('https://socialmediajavaapi.onrender.com/api/v1/comments/post/'+postId+'/before/10000000?limit=10')
            .then(response => {
    ...
    const handleLoadMoreComments = () =>{
        ...
        axios.get(`https://socialmediajavaapi.onrender.com/api/v1/comments/post/${postId}/before/${comments[comments.length - 1].id}?limit=10`)
         .then(response => {
            if(response.data.length < 10){
                setDisplayLoadMoreComment(false);
            }
    ...
    const handleShowReplies = (commentId) => {
      if (!replies[commentId]) {
        ...
        axios.get(`https://socialmediajavaapi.onrender.com/api/v1/comments/${commentId}/replies/after/0?limit=10`)
    ...
    const handleLoadMoreReplies = (commentId, lastReplyId) => {
      axios.get(`https://socialmediajavaapi.onrender.com/api/v1/comments/${commentId}/replies/after/${lastReplyId}?limit=10`)
        ...
        if (token) {
            try {
                const response = await axios.post(
                    `https://socialmediajavaapi.onrender.com/api/v1/comments/reply/post/${postId}/parent/${parentId}`,
    ...
    const deleteComment = async (commentId, isReply = false, parentId = null) => {
        ...
        const apiUrl = `https://socialmediajavaapi.onrender.com/api/v1/comments/delete/${commentId}`;
        try {
