...
const PostContent = ({postData}) => {
    ...
    const toggleLike = async (postId) => {
        ...
        try {
          const response = await axios.post(`https://socialmediajavaapi.onrender.com/api/v1/likes/post/${postId}`, {}, {
            headers: {
    ...
    const toggleBookmark = async (postId) => {
        ...
        const token = Cookies.get('token');
        try {
            const response = await axios.post(`https://socialmediajavaapi.onrender.com/api/v1/bookmarks/toggle?postId=${postId}`, {}, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
