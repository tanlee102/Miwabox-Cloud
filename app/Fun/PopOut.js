        ...
        this.conFirmFun("Management", "Do you want to ban this member?", async () => {
            ...
            setTimeout(async () => {
                try {
                    ...
                    const response = await axios.post(`https://socialmediajavaapi.onrender.com/api/v1/users/toggle-ban/${this.idMember}`, {}, {
                        headers: {
        ...
        if (this.Block == -1) {
            ...
            this.conFirmFun("Member", "Do you want to block this member? If blocked, they will not be able to reply to and like your future posts.", () => {
                ...
                setTimeout(async () => {
                    try {
                        const token = Cookies.get('token');
                        const response = await axios.post('https://socialmediajavaapi.onrender.com/api/v1/blocks/block?blockedId=' + this.idMember, {}
                            ,{
                                headers: {
                                  'Authorization': `Bearer ${token}`,
                                  'Content-Type': 'application/json'
    ...
    removeBlock = () => {
        this.conFirmFun("Member", "Do you want to unblock this member?", () => {
            ...
            setTimeout(async () => {
                try {
                    const token = Cookies.get('token');
                    const response = await axios.delete(`https://socialmediajavaapi.onrender.com/api/v1/blocks/unblock?blockedId=${this.idMember}`, {
                        headers: {
                            'Authorization': `Bearer ${token}`,
                            'Content-Type': 'application/json'
                        }
    ...
    setAdminRole = () => {
        this.conFirmFun("Management", "Do you want to set this member as ADMIN?", async () => {
            ...
            setTimeout(async () => {
                try {
                    const token = Cookies.get('token');
                    const response = await axios.post(`https://socialmediajavaapi.onrender.com/api/v1/users/role/admin/${this.idMember}`, {}, {
                        headers: {
                            'Authorization': `Bearer ${token}`,
                            'Content-Type': 'application/json'
                        }
