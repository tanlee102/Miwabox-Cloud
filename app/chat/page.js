...
const page = () => {
  ...
  const loadConversations = async (userId) => {
    try {
      const response = await axios.get('https://socialmediajavaapi.onrender.com/api/conversations?userId=' + userId);
  ...
  const loadMessages = async (conversationId, size, lastMessageId) => {
    try {
      ...
      ? `https://socialmediajavaapi.onrender.com/api/v1/messages?conversationId=${conversationId}&size=${size}&lastMessageId=${lastMessageId}`
      : `https://socialmediajavaapi.onrender.com/api/v1/messages?conversationId=${conversationId}&size=${size}`;
  ...
  useEffect(() => {
    ...
    const connect = () => {
      const socket = new SockJS('https://socialmediajavaapi.onrender.com/ws');
