...
async function getData(postId, userId) {
    const res = await fetch('https://socialmediajavaapi.onrender.com/api/v1/posts/v2/'+postId+'?userId='+userId)
    if (!res.ok) {
...
async function getRandomPosts(limit, postId) {
    const res = await fetch('https://socialmediajavaapi.onrender.com/api/v1/posts/randomExcludingId?limit=' + limit + '&excludePostId='+postId)
    if (!res.ok) {
